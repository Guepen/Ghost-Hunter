...
function _handleMainModuleRequest(reqPath, req, res) {
    ...
    if (!mainModulePath) {
        return _sendError(res, 'Could not find module "' + reqPath + '" from "' + opts.path + '"')
    }

    try {
        var dependencyTree = getDependencyLevels(mainModulePath)
    }
    catch (err) {
        return _sendError(res, 'in getDependencyLevels: ' + (err.message || err))
    }
    ...
    var response = [
        '__require__ = {',
            ...
            if (__require__.currentLevel.length) {
                return
            }
...
function _asString(fn) {
    return fn.toString()
}

function _handleModuleRequest(reqPath, res) {
    try {
        var code = _getModuleCode(res, reqPath)
    }
    catch (err) {
        return _sendError(res, err.stack || err)
    }
    ...
    function _getModuleCode(res, reqPath) {
        ...
        try {
            each(requireStatements, function (requireStmnt) {
                ...
                if (!depPath) {
                    throw 'Could not resolve module'
                }
